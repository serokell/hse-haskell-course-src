...
\begin{frame}{Пример: комбинатор $\Cx$}
 ...
 \[
  \infer[\mathrm{Abs}]
  {\vdash \lambda f. \lambda x. \lambda y. f yx : r_0} 
  {\infer[\mathrm{Abs}]
  {f: r_1 \vdash \lambda x. \lambda y. f y x : r_2}
  {\infer[\mathrm{Abs}]
  {f: r_1, x : r_3 \vdash \lambda y .f y x : r_4}{\infer[\mathrm{App}]{f : r_1, x: r_3, y : r_5 \vdash f y x : r_6}
  {\infer[\mathrm{App}]{f : r_1, y : r_5 \vdash f y : r_7}
  {f : r_1 \vdash f : r_9 & y : r_5 \vdash y : r_{10}}& 
  x : r_3 \vdash x : r_8}}}}
 \]
}}


\visible<3->{<<Желательные равенства>>:

{\small
\begin{tabular}{l@{\qquad}l@{\qquad}l}
\only<1-3>{$r_{10} \approx r_5$}\only<4->{{\color{gray}$r_{10} := r_5$}} & 
 \only<1-4>{$\only<1-3>{r_9}\only<4->{{\color{blue} r_1}} \approx \only<1-3>{r_{10}}\only<4->{{\color{blue} r_5}} \to r_7$}\only<5->{{\color{gray} $r_1 := r_5 \to r_7$}}  & \only<1-6>{$r_4 \approx r_5 \to r_6$}\only<7->{{\color{gray} $r_4 := r_5 \to r_6$}}  \\
 %
 \only<1-3>{$r_9 \approx r_1$}\only<4->{{\color{gray}$r_9 := r_1$}} &
 \only<1-5>{$r_7 \approx \only<1-3>{r_8}\only<4->{{\color{blue} r_3}} \to r_6$}\only<6->{{\color{gray} $r_7 := r_3 \to r_6$}} & \only<1-7>{$r_2 \approx r_3 \to \only<1-6>{r_4}\only<7->{{\color{blue} (r_5 \to r_6)}}$}\only<8->{{\color{gray} $r_2 := r_3 \to (r_5 \to r_6)$}} \\
 %
 \only<1-3>{$r_8 \approx r_3$}\only<4->{{\color{gray}$r_8 := r_3$}} & & 
 \only<1-8>{$r_0 \approx \only<1-4>{r_1}\only<5>{{\color{blue} (r_5 \to r_7)}}\only<6>{(r_5 \to {\color{blue} (r_3 \to r_6)})}\only<7->{(r_5 \to (r_3 \to r_6))} \to \only<1-7>{r_2}\only<8>{{\color{blue} (r_3 \to (r_5 \to r_6))}}$}\only<9->{{\color{red} $r_0 := (r_5 \to (r_3 \to r_6)) \to (r_3 \to (r_5 \to r_6))$}}
 \end{tabular}
...
{\small
 \[\hspace*{-1em}
  \infer[\mathrm{Abs}]
  {\vdash \lambda f. \lambda x. \lambda y. f yx : (r_5 \to (r_3 \to r_6)) \to (r_3 \to (r_5 \to r_6))} 
  {\infer[\mathrm{Abs}]
  {f: r_5 \to (r_3 \to r_6) \vdash \lambda x. \lambda y. f y x : r_3 \to (r_5 \to r_6)}
  {\infer[\mathrm{Abs}]
  {f: r_5 \to (r_3 \to r_6), x : r_3 \vdash \lambda y .f y x : (r_5 \to r_6)}{\infer[\mathrm{App}]{f : r_5 \to (r_3 \to r_6), x: r_3, y : r_5 \vdash f y x : r_6}
  {\infer[\mathrm{App}]{f : r_5 \to (r_3 \to r_6), y : r_5 \vdash f y : r_3 \to r_6}
  {f : r_5 \to (r_3 \to r_6) \vdash f : r_5 \to (r_3 \to r_6) & y : r_5 \vdash y : r_5}& 
  x : r_3 \vdash x : r_3}}}}
 \]
}
 
 
 \visible<2->{Вместо $r_5$, $r_3$, $r_6$ можно подставить произвольные типы: $\Cx : (A \to (B \to C)) \to (B \to (A \to C))$.}
 
 %FIXME: честный вывод с $\Cx$
 
 %FIXME: другой пример --- комбинатор $\Yx$
\end{frame}


\begin{frame}{Пример: комбинатор $\Yx$}
 
 $\Yx = \lambda f. \bigl( (\lambda x.  f(xx))
 (\lambda x. f(xx)) \bigr)$
 
 \visible<2->{{\small
 \[\hspace*{-1em}
 \infer[\mathrm{Abs}]{\vdash \lambda f. \bigl( (\lambda x.  f(xx))
 (\lambda x. f(xx)) \bigr) : r_0}
 {\infer[\mathrm{App}]{f: r_1 \vdash (\lambda x. f(xx)) (\lambda x. f(xx)) : r_2}
 {\infer[\mathrm{Abs}]{f : r_1 \vdash \lambda x. f(xx) : r_3}
 {\infer[\mathrm{App}]{f:r_1, x:r_5 \vdash f(xx) : r_6}
 {f : r_1 \vdash f : r_7 & 
{\only<3->{\color{red}} \infer[\mathrm{App}]{x : r_5 \vdash xx : r_8}{x : r_5 \vdash x : r_9 & x : r_5 \vdash x : r_{10}}}}} & 
 \infer{f : r_1 \vdash \lambda x. f(xx) : r_4}{\ldots}}}
 \]
 }}
 
 \visible<4->{
\only<1-4>{$r_{10} \approx r_5$}\only<5->{{\color{gray} $r_{10} := r_5$}} \\
\only<1-4>{$r_{9} \approx r_5$}\only<5->{{\color{gray} $r_9 := r_5$}} \\
\only<1-4>{$r_{9} \approx r_{10} \to r_8$}
\only<5>{${\color{blue} r_5} \approx {\color{blue} r_5} \to r_8$}\only<6->{{\color{red} $r_5 \approx r_5 \to r_8$}}\\
  \ldots
 }
\end{frame}
...
\visible<2->{В случаях 3 и 4 алгоритм сразу останавливается.}
\end{frame}
