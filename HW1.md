...
Определить функцию прибавления единицы:
```haskell
succChurch :: Nat a -> Nat a
succChurch = undefined
```

Реализовать умножение и сложение нумералов Черча:
```haskell
...
Реализовать функцию, которая по нумералу Черча возвращает соответствующее ему целое число:
```haskell
churchToInt :: Nat Integer -> Integer
churchToInt = undefined
```

Функция должна удовлетворять следующим естественным условиям:
...
В задании требуется предоставить дерево синтаксического разбора (или его подобие) данного терма, каждая вершина которого аннотипована типами.
...
10.3. Пусть у нас есть запись, которая хранит информацию о человеке: имя, фамилию, возраст
```haskell
data Person
  = Person
  { firstName :: String
...
```
Реализуйте функцию `updateLastName person1 person2`, которая меняет фамилию `person2` на фамилию `person1`
```haskell
updateLastName
  :: Person
  -> Person
