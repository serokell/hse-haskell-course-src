...
On the previous seminar
  \begin{itemize}
    \item we worked with computational contexts using such type classes
    as \verb"Functor", \verb"Foldable", and \verb"Traversable"
  \end{itemize}

\onslide<2->{
Today we
\begin{itemize}
  \item dive into monads deeply}
\end{itemize}
\end{frame}
...
\begin{frame}
  ...
  We are going to extend pure functions \verb"a -> b", we would like to extend them to computations with effects:
  \begin{itemize}
    \item A computation may fail: \verb"a -> Maybe b"
    \item A computation yields more than one result: \verb"a -> [b]"
    \item A computation either succeeds or yields an error: \verb"a -> Either e b"
    \item A computation with log: \verb"a -> (s, b)"
    ...
    \item A generalised version of such a function is a Kleisli function that has type \verb"a -> m b"
  \end{itemize}
\end{frame}
...
If one needs to provide a uniform interface to deal with Kreisli functions, then this interface should satisfy the following two requirements.
  ...
  \begin{enumerate}
    ...
    \item Kleisli maps should composable:
...
If we erase all \verb"m" we get a type of this reverse application operator:
  \begin{minted}{haskell}
  (&) :: a -> (a -> b) -> b
  x & f = f x
  \end{minted}
...
\begin{frame}[fragile]
  ...
  One may observe this analogy, here are the direct \verb"fmap", \verb"(<*>)" and flipped monadic bind:
  \begin{minted}{haskell}
...
In fact, \verb">>=" works here similarly to \verb"(&)".
\end{frame}
...
\end{frame}
  ...
  \begin{frame}[fragile]
    ...
    \alert{Wow, we invited imperative programming!}
\end{frame}
...
\begin{frame}[fragile]
  ...
  The \verb"Maybe" data type is one of the simplest non-trivial monads. This monad represents an the simplest error effect. An error is represented with the \verb"Nothing" flag
  \begin{minted}{haskell}
...
\begin{itemize}
\item The \verb"Monad" instance is the following one:
  ...
  \end{minted}
\item The function \verb"k :: a -> [b]" maps to the function that has type \verb"[a] -> [b]".
We apply \verb"k" to every element of \verb"[a]" and concat all results.
\end{itemize}
\end{frame}
