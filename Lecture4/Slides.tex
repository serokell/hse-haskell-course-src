...
\onslide<2->{
  \begin{itemize}
    \item Expressions like \verb"(a,b)", \verb"[]", and \verb"(x : xs)" are called \emph{patterns}
    \item One needs to check that constructors \verb"(,)" and \verb"( : )" are relevant.
    \item Consider \verb"swap (45, True)". Variables \verb"a" and \verb"b" are binded with the values \verb"45" and \verb"True".
    \item Consider \verb"lenght [1,2,3]". Variables \verb"x" and \verb"xs" are binded with the values \verb"1" and \verb"[2,3]"
  \end{itemize}
  }
\end{frame}
...
\end{exampleblock}
  ...
  \item The type of the \verb"Point" type constructor
  \metroset{block=fill}
  \begin{exampleblock}{Example}
  \begin{minted}{haskell}
  > :type Point
  ...
  \end{minted}
  \end{exampleblock}
  \item \verb"Point" is a type operator. One also has a type (kind) system for types:
  \metroset{block=fill}
  \begin{exampleblock}{Example}
  \begin{minted}{haskell}
  > :k Point
...
\end{exampleblock}
  ...
  \item In fact, this construction is a sugar for the following one:
  \metroset{block=fill}
  \begin{exampleblock}{Example}
  \begin{minted}{haskell}
  dupHead :: [a] -> [a]
...
\begin{frame}[fragile]
  ...
  \end{exampleblock}
    ...
    \end{minted}
    \onslide<2->{
    \item The implementation of the left fold function might be optimised. Here we have a thunk
...
\begin{frame}
  \frametitle{The definition of a left fold}
  One may visualise \verb"foldl" in the same manner:
  \xymatrix{
  & \verb":" \ar[dl] \ar[dr] &&&&&&&& \verb"f" \ar[dl] \ar[dr] \\
...
\begin{frame}
  ...
  \begin{itemize}
    \item Evaluation process in Haskell is lazy.
